		...
		if(!empty($_FILES['myfile']['name']) && isset($_FILES['myfile'])){
			...
			$size1 = $_FILES['myfile']['size'];
				if (move_uploaded_file($file1, $destination1) )
				{
					...
					$sql = "INSERT INTO `internship_data`(`roll_no`, `topic`,`year_new`, `starting_date`, `end_date`,`company_name`, `approval_status`, `completion_status`, `certificate`,`duration`,`myfile`,`comment`) VALUES ('$roll_no','$topic','$year','$starting_date','$end_date','$company_name','Pending','None','Not Uploaded Yet',$duration,'".$destination1."','No comment')";
					if ($conn->query($sql)) 
					{
			...
			echo "Hi";
			$sql = "INSERT INTO `internship_data`(`roll_no`, `topic`,`year_new`, `starting_date`, `end_date`,`company_name`,`approval_status`, `completion_status`, `certificate`,`duration`,`myfile`,`comment`) VALUES ('$roll_no','$topic','$year','$starting_date','$end_date','$company_name','Pending','None','Not Uploaded Yet',$duration,'','No comment')";
